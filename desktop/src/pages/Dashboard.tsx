...
import { Activity, Calendar, Search, ChevronRight, Zap, LogOut, Loader2 } from 'lucide-react';
        ...
        <div className="min-h-screen bg-slate-950 text-white flex">
            ...
            <aside className="w-64 bg-slate-900 border-r border-slate-800 p-6 flex flex-col">
                ...
                <nav className="space-y-2 flex-1">
                    <NavItem active icon={<Calendar />} label="Partidos de Hoy" onClick={() => navigate('/')} />
                    <NavItem icon={<Activity />} label="Mis Análisis (Historial)" onClick={() => navigate('/history')} />
                    <NavItem icon={<Zap />} label="Anti-FOMO" onClick={() => navigate('/anti-fomo')} />
                </nav>
                ...
                <div className="pt-6 border-t border-slate-800">
                    ...
                    <div className="flex items-center gap-3 px-2">
                        <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500">U</div>
                        <div className="text-sm">
                            <div className="font-medium">Usuario</div>
                            <div className="text-emerald-500 text-xs font-bold">Premium</div>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                <div className="p-8 pb-0">
                    <header className="flex justify-between items-center mb-8">
                        <div>
                            <h1 className="text-2xl font-bold mb-1">Dashboard Principal</h1>
                            <p className="text-slate-400 text-sm">Resumen de partidos y oportunidades de hoy.</p>
                        </div>
                        <div className="flex gap-3 relative z-50">
                            <div className="relative w-64">
                                ...
                                {(searchResults.length > 0 || (searchQuery.length >= 2 && !isSearching)) && (
                                    <div className="absolute top-full mt-2 w-full bg-slate-900 border border-slate-800 rounded-xl shadow-xl overflow-hidden">
                                        {searchResults.length > 0 ? (
                                            ...
                                            <div className="px-4 py-3 text-sm text-slate-500 text-center">No se encontraron jugadores</div>
                                        )}
                                    </div>
                                )}
                            </div>
